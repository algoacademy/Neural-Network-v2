...
Mari kita mulai pengalaman penerapan neural network kita dengan menyiapkan dataset sebagai langkah pertama. Anda akan menggunakan `sign-language-mnist` dataset yang disimpan dalam folder `data_input`dengan nama file `sign-mnist-train.csv` dan `sign-mnist-test.csv`. Data terdiri dari satu set train dan test yang menyimpan gambar bahasa isyarat (sign language) berukuran 28 x 28 pixel untuk 24 kategori yang berbeda. 
...
___
3. Model neural network dibangun untuk mengoptimalkan (meminimalkan) error, error (**loss function**) apa yang digunakan pada kasus klasifikasi biner?
  - [ ] Sum Square Error
  - [ ] Binary Crossentropy
...
___
4. Dalam membangun arsitektur model neural network, kita mengatur beberapa parameter. Di bawah ini merupakan pertimbangan menggunakan angka-angka tersebut, **KECUALI**? 
...
___
5. Dalam fitting model di atas, apa arti dari `epoch = 10`?
...
Silahkan jawab pertanyaan di bawah ini.
___
7. Hidden layer merupakan tempat dimana informasi dari data diekstraksi. Apa yang dapat Anda simpulkan dari `model_bigger` dan `model_tuning` mengenai hidden layer?
...
___
8. Dari 3 model yang telah dibuat (`model_base`, `model_bigger` dan `model_tuning`), model mana yang paling baik untuk kita pilih?
